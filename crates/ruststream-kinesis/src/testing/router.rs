//! Subscription registry and fanout for the in-process Kinesis stand-in.
//!
//! Core routing only: an exact-name match fans a published message out to every live
//! subscription on that name, and a per-name log records traffic for assertions. Kinesis's own
//! semantics (shards, leases, checkpoints, retention, replay) are transport behaviour and are
//! not simulated here.
